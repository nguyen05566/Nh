#!/usr/bin/env python3
"""
Caro Bot - Sử dụng OpenRouter AI (mô hình miễn phí) để chọn nước đi
Fallback: Heuristic thuần túy khi API lỗi
"""
...
try:
    ...
    import subprocess
    subprocess.run(["pip", "install", "websockets", "requests", "-q", "--break-system-packages"])
    import websockets
    import requests
...
# OpenRouter config
OPENROUTER_API_KEY = "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"
OPENROUTER_URL = "https://openrouter.ai/api/v1/chat/completions"

# Danh sách mô hình miễn phí theo thứ tự ưu tiên (Owl Alpha test OK, nhanh ~3s)
FREE_MODELS = [
...
# Mô hình đang dùng (sẽ tự chuyển nếu lỗi)
current_model_idx = 0
current_model = FREE_MODELS[0]

# Thời gian chờ tối đa cho API (giây)
API_TIMEOUT = 10
...
logging.basicConfig(level=logging.INFO,
                    ...
                    datefmt="%H:%M:%S",
                    force=True)
# Force flush mọi log message
logging.getLogger().handlers[0].flush = lambda: sys.stdout.flush() if hasattr(sys.stdout, 'flush') else None
log = logging.getLogger("caro-ai")

# Thêm StreamHandler với flush ngay
import sys
sh = logging.StreamHandler(sys.stdout)
sh.setFormatter(logging.Formatter("%(asctime)s %(message)s", datefmt="%H:%M:%S"))
sh.flush = lambda: sys.stdout.flush()
log.handlers = [sh]
...
# Điểm số cho các pattern (heuristic fallback)
PATTERN_SCORES = {
    'FIVE': 100_000_000,
    'OPEN4': 5_000_000,
    'HALF4': 500_000,
...
# ========================== OPENROUTER AI ==========================
def switch_model():
    """Chuyển sang mô hình miễn phí tiếp theo"""
    ...
    log.info(f"[AI] Switched to model: {current_model}")


def board_to_text(board, my_symbol) -> str:
    """Chuyển bàn cờ thành text cho AI đọc"""
    symbol_map = {EMPTY: '.', CROSS: 'X', CIRCLE: 'O'}
    my_char = 'X' if my_symbol == CROSS else 'O'
    
    lines = []
    # Header cột
    header = "   " + "".join(f"{i:2d}" for i in range(board.width))
    lines.append(header)
    
    for y in range(board.height):
        row = f"{y:2d} "
        for x in range(board.width):
            row += f" {symbol_map[board.get(x, y)]}"
        lines.append(row)
    
    board_text = "\n".join(lines)
    return board_text, my_char


def ask_ai_for_move(board, my_symbol) -> Optional[Tuple[int, int]]:
    """Hỏi AI qua OpenRouter để chọn nước đi"""
    global current_model
    
    board_text, my_char = board_to_text(board, my_symbol)
    opponent_char = 'O' if my_char == 'X' else 'X'
    
    # Đếm số nước đã đi
    move_count = len(board.history)
    
    # Lấy danh sách ô trống gần quân cờ
    neighbors = board.get_neighbors(radius=2)
    if not neighbors:
        neighbors = [(board.width // 2, board.height // 2)]
    
    # Tạo danh sách tọa độ gợi ý
    candidate_str = ", ".join(f"({x},{y})" for x, y in neighbors[:30])
    
    prompt = f"""You are an expert Caro (Gomoku/Tic-Tac-Toe on 15x19 board) player.
You play as '{my_char}'. Opponent is '{opponent_char}'.
You need to get 5 in a row (horizontal, vertical, or diagonal) to win.

Current board ({board.width}x{board.height}):
{board_text}

Moves played so far: {move_count}
Your symbol: {my_char}

Top candidate positions: {candidate_str}

Analyze the board and choose the BEST move. Consider:
1. Can you win immediately? (5 in a row)
2. Can you block opponent's winning move?
3. Can you create an open-4 or double threat?
4. Can you block opponent's open-3 or open-4?

Respond with ONLY the coordinates in this exact format: MOVE x,y
Example: MOVE 7,9"""
    ...
    try:
        resp = requests.post(
            ...
            headers={
                ...
                "HTTP-Referer": "https://gamevh.net",
                "X-Title": "Caro Bot"
            },
            json={
                "model": current_model,
                "messages": [
                    {"role": "system", "content": "You are an expert Caro/Gomoku AI player. Always respond with only: MOVE x,y"},
        ...
        if resp.status_code == 429:
            log.warning(f"[AI] Rate limited on {current_model}, switching...")
            switch_model()
            return None
        ...
        log.info(f"[AI] {current_model} responded: {content[:100]}")
        
        # Parse MOVE x,y hoặc (x,y) hoặc x,y
        move = parse_ai_move(content, board)
        if move:
            x, y = move
            if 0 <= x < board.width and 0 <= y < board.height and board.get(x, y) == EMPTY:
                log.info(f"[AI] Valid move: ({x},{y})")
                return move
            else:
                log.warning(f"[AI] Invalid position: ({x},{y}) - out of bounds or occupied")
                return None
        
        log.warning(f"[AI] Could not parse move from: {content[:100]}")
        return None
        
    except requests.exceptions.Timeout:
        log.warning(f"[AI] Timeout ({API_TIMEOUT}s) on {current_model}")
        switch_model()
        return None
...
def parse_ai_move(content: str, board) -> Optional[Tuple[int, int]]:
    """Parse nước đi từ phản hồi AI"""
    # Thử các pattern khác nhau
    patterns = [
        r'MOVE\s+(\d+)\s*,\s*(\d+)',         # MOVE x,y
        r'\((\d+)\s*,\s*(\d+)\)',               # (x,y)
        r'(\d+)\s*,\s*(\d+)',                    # x,y
        r'(\d+)\s+(\d+)',                         # x y
        r'row\s*(\d+)\s*col\s*(\d+)',            # row x col y
        r'x\s*=\s*(\d+).*?y\s*=\s*(\d+)',        # x=X, y=Y
    ]
    ...
    for pattern in patterns:
        ...
        if match:
            try:
                ...
                # Kiểm tra xem val1,val2 có hợp lệ không
                if 0 <= val1 < board.width and 0 <= val2 < board.height:
                    return (val1, val2)
                # Thử ngược lại (có thể AI trả về y,x)
                if 0 <= val2 < board.width and 0 <= val1 < board.height:
                    return (val2, val1)
...
def pick_best_move_heuristic(board: Board, my_symbol: int) -> Tuple[int, int]:
    """Chọn nước đi tốt nhất dựa trên heuristic (fallback)"""
    opponent = CROSS if my_symbol == CIRCLE else CIRCLE
    move_count = len(board.history)
...
def pick_best_move(board: Board, my_symbol: int) -> Tuple[int, int]:
    """Chọn nước đi: thử AI trước, fallback heuristic"""
    move_count = len(board.history)
    
    # Nước đầu và thứ hai luôn dùng heuristic (quá đơn giản để cần AI)
    if move_count <= 1:
        return pick_best_move_heuristic(board, my_symbol)
    
    # Kiểm tra nước thắng/chặn thắng ngay bằng heuristic (quan trọng, không phụ thuộc AI)
    opponent = CROSS if my_symbol == CIRCLE else CIRCLE
    
    win_move = find_pattern(board, my_symbol, 'FIVE')
    if win_move:
        log.info(f"[INSTANT WIN] {win_move}")
        return win_move
    
    block_win = find_pattern(board, opponent, 'FIVE')
    if block_win:
        log.info(f"[INSTANT BLOCK] {block_win}")
        return block_win
    
    # Thử hỏi AI
    ai_move = ask_ai_for_move(board, my_symbol)
    if ai_move:
        return ai_move
    
    # Fallback: heuristic
    log.info("[FALLBACK] Using heuristic (AI unavailable)")
    return pick_best_move_heuristic(board, my_symbol)
...
class CaroBot:
    ...
    async def do_move(self):
        ...
        # Chọn nước đi (AI + heuristic fallback)
        x, y = pick_best_move(self.board, self.my_symbol)
        elapsed = time.time() - start
        
        pos = self.board.xy_to_pos(x, y)
        log.info(f"MOVE ({x},{y}) took {elapsed:.2f}s [model={current_model}]")
    ...
    async def handle_start(self, r: BinaryReader):
        ...
        log.info(f"=== GAME {self.total_games} === Me={'X' if self.my_symbol == CROSS else 'O'} Model={current_model}")
    ...
    async def handle_gameover(self, r: BinaryReader):
        ...
        log.info(f"STATS: {self.total_games}G {self.wins}W {self.losses}L {self.draws}D WR={win_rate:.0f}% {minutes:.0f}m AI_model={current_model}")
        self.save_stats()
    ...
    async def run(self):
        ...
        log.info(f"Caro AI Bot started - {RUNTIME // 3600} hours")
        log.info(f"Using OpenRouter model: {current_model}")
        log.info(f"Free models available: {', '.join(FREE_MODELS)}")

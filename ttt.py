...
class PikafishBot:
    ...
    def _handle_start_match(self, msg):
        ...
        try:
            ...
            if my_slot_id == first_turn_slot_id:
                ...
                # FIX SẬP MẠNG: Đưa việc tính toán vào Thread riêng để giải phóng luồng nhận tin nhắn WebSocket
    ...
    def _handle_move(self, msg):
        try:
            ...
            # FIX LỖI ĐƠ LUỢT: Đồng bộ chính xác, loại bỏ trùng nước đi từ _make_auto_move dội về
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                # FIX GIỮ GHẾ: Vòng lặp tối ưu kiểm tra mỗi giây để tìm bàn nhanh chóng
                while self.connected:
                    if not self.in_game:
                        self.send_quick_play()
                        for _ in range(6):

...
Tích hợp:
1. Tự động lưu và cập nhật thông tin tài khoản an toàn qua file bot_session.json (Tránh hỏng file code)
2. Tự động gửi lệnh SET_READY (434) sau khi kết thúc trận đấu để đấu ván tiếp theo (Delay 5s).
3. Tự động gửi lệnh SET_READY (434) ngay khi vừa vào bàn chơi thành công (Delay 5s).
4. Tự động phát hiện đối thủ vào bàn, đợi đúng 5 giây rồi mới bấm Ready (Tránh Ready treo khi bàn trống).
"""
...
BOT_DEPTH = 10  # Tăng lại lên 15 cho tối ưu nước đi
...
class PikafishBot:
    ...
    def _init_engine(self):
        ...
        # 1. Thư mục hiện tại (./) - Tốt nhất cho GitHub Actions/VPS
        # 2. Thư mục home (~) - Tốt cho Termux/Linux
        # 3. Các đường dẫn cũ (dự phòng)
        possible_paths = [
            "./pikafish",
        ...
        try:
            ...
            # Tự động tìm file NNUE trong cùng thư mục với file pikafish
            base_dir = os.path.dirname(os.path.abspath(pikafish_path))
            possible_nnue = ["./pikafish.nnue", os.path.join(base_dir, "pikafish.nnue"), os.path.expanduser("~/pikafish.nnue")]
            nnue_path = next((p for p in possible_nnue if os.path.isfile(p)), None)
            ...
            # Sử dụng lệnh viết liền để kích hoạt NNUE
    ...
    def _handle_slot_changed(self, msg):
        try:
            ...
            # Cập nhật vị trí ghế nếu là chính mình
            if player_id == CURRENT_PLAYER_ID: 
                self.board.my_slot_id = slot_id
            else:
                # --- ĐÃ TÍCH HỢP: Nếu có người chơi khác (player_id > 0) vừa vào bàn ---
    ...
    def _handle_set_turn(self, msg):
        try:
            slot_id = msg.read_byte()
            if slot_id != -1: self.board.is_my_turn = (slot_id == self.board.my_slot_id)
        except: pass

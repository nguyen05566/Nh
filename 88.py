#!/usr/bin/env python3
"""
Xiangqi Bot - gamevh.net (Bản Hoàn Chỉnh Sửa Lỗi Mất Kết Nối & Tối Ưu RAM Siêu Tốc)
"""
...
class TrendAnalyzer:
    ...
    def parse_line(self, line_str):
        # 1. Quét sát cục (Mate) trước để tránh Bot đi vòng vo khi sắp thắng
        ...
        # 2. Phân tích điểm cp thông thường
    ...
    def select_best_trend_move(self):
        ...
        # ƯU TIÊN SÁT CỤC DƯƠNG: Nhánh nào dứt điểm được trận đấu thì đi ngay!
        for move, data in self.pv_ram_cache.items():
            if data["mate_in"] is not None and data["mate_in"] > 0:
                print(f"[RAM-MATE] 🔥 Phát hiện đường sát cục tuyệt đối! Bỏ qua xu hướng, dứt điểm: {move}")
                return move
        ...
        if is_negative:
            ...
            for move, data in self.pv_ram_cache.items():
                ...
                if recovery_rate > max_recovery:
                    ...
                    best_move = move
            print(f"[RAM-LEARN] Đang lép vế ({int(avg_score)}). Ép chọn nước phòng thủ tối ưu: {best_move}")
        else:
            max_growth = -999999
            for move, data in self.pv_ram_cache.items():
...
class PikafishBot:
    ...
    def _init_engine(self):
        ...
        try:
            ...
            self.engine = True
            print(f"[ENGINE] ✅ Sẵn sàng chạy mượt mà ở MultiPV=3.")
        except Exception as e:
    ...
    def _handle_gameover(self, msg):
        print("[GAME] 🏁 Trận đấu kết thúc. Tiến hành giải phóng RAM và đặt lại trạng thái...")
        self.fixed_pawn_positions.clear()
        self.board.reset()
        self.board.is_playing = False
        self.board.is_my_turn = False
        
        # FIX CỐT LÕI: Đưa về False để vòng lặp run() tiếp tục tìm trận mới, tránh treo máy ngắt mạng
        self.in_game = False  
        self._joining_table = False
        self.last_action_timestamp = time.time()
        
        # Làm sạch rác bộ nhớ của Engine sau trận đấu
        if getattr(self, '_engine_proc', None) and self._engine_proc.poll() is None:
            self._fsf_cmd("ucinewgame")
            self._fsf_cmd("isready")

        # Gửi Ready chuẩn mực, loại bỏ cơ chế luồng chồng lặp
        def delay_ready():
            time.sleep(2.0)
            if self.connected and not self.board.is_playing:
                self.send_ready(1)
    ...
    def _make_auto_move(self):
        ...
        # Bộ lọc RAM thông minh bốc nước đi tối ưu dựa trên Sát cục / Xu hướng
        trend_move = self.trend_analyzer.select_best_trend_move()
        if trend_move and best_move not in ["(none)", "0000"]:
            print(f"[RAM-LEARN] 🧠 Ép chọn nước đi tối ưu cấu trúc dữ liệu: '{trend_move}' (Thay cho '{best_move}')")
            best_move = trend_move

        if best_move in ["(none)", "0000"]:
            print("\n[HỆ THỐNG TÀN CUỘC] ⚠️ Hết nước đi hợp lệ.")
            self.board.is_my_turn = False
            return
            ...
            try:
                if self.connected and self.board.is_playing:
                    if time.time() - self.last_action_timestamp > 180:
                        print("[TIMEOUT] Quá lâu không phản hồi, chủ động tái kết nối...")
                        if self.ws: self.ws.close()
                        time.sleep(2)

                if not self.connected:
                    print("[NETWORK] Đang thực hiện kết nối lại hệ thống...")
                ...
                time.sleep(2) # Tốc độ kiểm tra vòng lặp chính nhạy bén 2 giây

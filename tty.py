...
class PikafishBot:
    ...
    def _find_king_pos(self, fen):
        """Tìm vị trí tướng của bot trong FEN string."""
        king_char = 'K' if self.board.is_red else 'k'
        board_part = fen.split(' ')[0]
        ...
        for fen_row_idx, row_str in enumerate(rows):
            ...
            for ch in row_str:
                if ch.isdigit():
                    ...
                    if ch == king_char:
                        # Chuyen FEN row -> game pos
                        # Gamevh: row=0 la hang tren cung, FEN: row=0 la hang duoi (den)
                        game_row = 9 - fen_row_idx
                        game_pos = game_row * 9 + col
                        # Tien 1 nuoc: target_row = game_row + 1 (neu do) hoac game_row - 1 (neu den)
                        if self.board.is_red:
                            target_row = game_row + 1
                        else:
                            target_row = game_row - 1
                        target_pos = target_row * 9 + col
                        return game_pos, target_pos
                    col += 1
        return None, None
    ...
    def _make_auto_move(self):
        ...
        # Nuoc dau tien: len tuong 1 nuoc
        if not moves or len(moves) == 1:
            src, dst = self._find_king_pos(fen)
            if src is not None:
                print(f"[BOT] Nuoc dau tien - len tuong: {src} -> {dst}")
                time.sleep(1)
                self.send_play(src, dst)
                return

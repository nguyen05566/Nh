...
Fix:
  1. GAMEOVER → quay về sảnh (send_enter_place) trước khi tìm bàn mới
  2. Throttle QUICK_PLAY 10 giây, tránh spam server
  3. Cờ _returning_to_lobby chặn race condition
"""
...
BOT_DEPTH = int(os.environ.get('BOT_DEPTH', '13'))
...
class PikafishBot:
    def __init__(self, depth=20):
        ...
        self.in_game = False
        # ====== FIX #3: Thêm cờ chống race condition ======
        self._returning_to_lobby = False
        # ====== FIX #2: Throttle QUICK_PLAY ======
        self._last_quick_play_time = 0
        self._QUICK_PLAY_INTERVAL = 10  # Tối thiểu 10 giây giữa 2 lần
        self._init_engine()
    ...
    def get_best_move(self, fen, moves):
        try:
            ...
            _go_start = time.time()
            _go_timeout = 30  # Timeout 30 giây cho từng nước đi
            while True:
    ...
    def _handle_enter_place_response(self, msg):
        if msg.read_byte() == 0:
            if not self.in_game:
                print("[PLACE] Đã vào sảnh chờ. Sẵn sàng tìm bàn mới.")
            else:
    ...
    def _handle_quick_play_response(self, msg):
        if msg.read_byte() == 0:
            self.in_game = False
            self._returning_to_lobby = False  # Đang vào bàn, hủy cờ lobby
    ...
    def _handle_gameover(self, msg):
        print("[GAME] 🏁 Trận đấu kết thúc. Bắt đầu quay về sảnh...")
        self.board.is_playing = False
        self.board.is_my_turn = False
        self.in_game = False
        self._returning_to_lobby = True

        def _go_back_to_lobby():
            time.sleep(2)
            print("[GAME] 🔄 Đang gửi yêu cầu vào sảnh chờ...")
            self.send_enter_place()
            time.sleep(2)
            print("[GAME] ✅ Đã về sảnh, sẵn sàng tìm bàn mới.")
            self._returning_to_lobby = False
            self._last_quick_play_time = 0

        threading.Thread(target=_go_back_to_lobby, daemon=True).start()

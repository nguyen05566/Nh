...
(Bản Hoàn Chỉnh - Đã sửa lỗi BOT đi 1 nước rồi dừng)
"""
...
class XiangqiBoardTracker:
    ...
    def get_current_fen(self):
        # SỬA LỖI TẠI ĐÂY: Xác định chính xác lượt đi hiện tại dựa trên số nước đã đi
...
class MultiPVSelector:
    ...
    def select_move(self):
        ...
        if len(viable_moves) > 1:
            ...
            print(f"[SMART-PV] 🎲 Đa dạng hóa nước đi. Chọn: {chosen} trong {len(viable_moves)} nhánh.")
            return chosen
        else:
...
class PikafishBot:
    ...
    def _make_auto_move(self):
        ...
        try:
            ...
            book_key = f"{fen}_{'_'.join(moves)}"  # Đảm bảo phân biệt trạng thái dựa trên lịch sử đi
            ...
            if is_in_book:
                ...
                raw_bestmove_line = self.get_best_move(fen, moves, fixed_positions=fixed, time_limit=4000)
            ...
            try:
                ...
                if self.board.is_my_turn and self.board.is_playing:
                    ...
                    # CẬP NHẬT TRỰC TIẾP LỊCH SỬ ĐI CỦA CHÍNH MÌNH VÀO BOARD TRACKER
